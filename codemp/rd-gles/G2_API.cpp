...
void RestoreGhoul2InfoArray()
{
	if (singleton == NULL)
	{
		Com_Printf("singleton == NULL\n");
		// Create the ghoul2 info array
		Com_Printf("TheGhoul2InfoArray\n");
		TheGhoul2InfoArray();
		Com_Printf("ri->PD_Load\n");
		size_t size;
		const void *data = ri->PD_Load(PERSISTENT_G2DATA, &size);
		if ( data == NULL ) {
			Com_Printf("data == NULL\n");
			return;
		}
		Com_Printf("singleton->Deserialize\n");
...
{
	//LOGI("G2_SetupModelPointers %x",ghlInfo);
...
{
	//LOGI("G2_SetupModelPointers many ");
	...
	//LOGI("G2_SetupModelPointers many retured");

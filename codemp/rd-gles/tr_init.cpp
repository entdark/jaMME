...
//entTODO: remove them after porting mme additions
...
static void R_LevelShot( void ) {
	...
	source = (byte *)ri->Hunk_AllocateTempMemory( glConfig.vidWidth * glConfig.vidHeight * 4 );
...
//	Swap_Init();
	...
	//entTODO: remove it after porting mme additions
	...
	R_InitImages();
	ri->Printf(PRINT_ALL,"R_InitShaders\n");
	R_InitShaders(qfalse);
	ri->Printf(PRINT_ALL,"R_InitSkins\n");
	R_InitSkins();
	ri->Printf(PRINT_ALL,"R_TerrainInit\n");
	R_TerrainInit(); //rwwRMG - added
	ri->Printf(PRINT_ALL,"R_InitFonts\n");
	R_InitFonts();
	ri->Printf(PRINT_ALL,"R_ModelInit\n");
	R_ModelInit();
//	re.G2VertSpaceServer = &IHeapAllocator_singleton;
	ri->Printf(PRINT_ALL,"R_InitDecals\n");
	R_InitDecals ( );
	ri->Printf(PRINT_ALL,"R_InitWorldEffects\n");
	R_InitWorldEffects();

	int	err = qglGetError();
	if ( err != GL_NO_ERROR )
		ri->Printf( PRINT_ALL,  "glGetError() = 0x%x\n", err);
	ri->Printf(PRINT_ALL,"RestoreGhoul2InfoArray\n");
...
//	assert( rimp );
	...
	ri->Printf(PRINT_ALL,"Done GetRefAPI\n");
	return &re;
}

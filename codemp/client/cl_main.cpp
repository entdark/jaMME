...
===============
CL_WriteAudio
similar to CL_WriteAVIAudioFrame with fixes for separated audio
===============
*/
/*void CL_WriteAudio( const byte *pcmBuffer, int size ) {
  ...
  frameRate = (float)((float)cl_avidemo->integer / (float)mme_blurFrames->integer);

  audio.rate = dma.speed;
  audio.format = WAV_FORMAT_PCM;
  audio.channels = dma.channels;
  audio.bits = dma.samplebits;
  audio.sampleSize = ( audio.bits / 8 ) * audio.channels;

  if( audio.rate % frameRate )
  {
    int suggestRate = frameRate;

    while( ( audio.rate % suggestRate ) && suggestRate >= 1 )
      suggestRate--;
  ...
  if( bytesInBuffer >= (int)ceil( (float)audio.rate / (float)frameRate ) * audio.sampleSize ) {

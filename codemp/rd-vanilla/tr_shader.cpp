...
static int Shader_CompressBracedSection( char **data_p, char **name, char **text, int *nameLength, int *textLength ) { // drakkar - optimized sub-parse function
	...
	*textLength = 0;
		...
		if( c <= '/' || c >= '{' )	// skip lot of conditions if c is regular char
		{
			...
			if( c == '{' || c == '}' ) {
				if( c == '{' && !*name ) {
					*name = *data_p;
					if( *(*name) <= ' ' ) (*name)++;
					*nameLength = (intptr_t)out - (intptr_t)*name;
	...
	if( *text ) *textLength = (intptr_t)out - (intptr_t)*text;	// compressed text length

	c = (intptr_t)out - (intptr_t)*data_p;						// uncompressed chars parsed
...
static void ScanAndLoadShaderFiles( const char *path )
{
	...
	for ( i = 0; i < numShaderFiles; i++ )
	{
		...
		while(1)
		{
			...
			if(!SkipBracedSection2((const char **)&p, 1))
			{

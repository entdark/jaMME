...
void R_ScreenShotMME_f (void) {
	...
	if( ri.Cmd_Argc() == 2 )
	{
		if( !strcmp( ri.Cmd_Argv(1), "left") )
			stereo = 1;
		else if(!strcmp( ri.Cmd_Argv(1), "right"))
			stereo = 2;
	}
...
//	mme_pip = ri.Cvar_Get( "mme_pip", "0", CVAR_CHEAT );	//animated gay porn :s
...
*/
	...
	ri.Cmd_AddCommand( "screenshot_mme", R_ScreenShotMME_f );

...
static void R_Bloom_InitTextures( void )
{
	...
	data = (byte *)ri.Hunk_AllocateTempMemory( bloom.screen.width * bloom.screen.height * 4 );
	Com_Memset( data, 0, bloom.screen.width * bloom.screen.height * 4 );
	bloom.screen.texture = R_CreateImage( "***bloom screen texture***", data, bloom.screen.width, bloom.screen.height, GL_RGBA, qfalse, qfalse, qfalse, GL_CLAMP );
	ri.Hunk_FreeTempMemory( data );

	data = (byte *)ri.Hunk_AllocateTempMemory( bloom.effect.width * bloom.effect.height * 4 );
	Com_Memset( data, 0, bloom.effect.width * bloom.effect.height * 4 );
	bloom.effect.texture = R_CreateImage( "***bloom effect texture***", data, bloom.effect.width, bloom.effect.height, GL_RGBA, qfalse, qfalse, qfalse, GL_CLAMP );
	ri.Hunk_FreeTempMemory( data );
	bloom.started = qtrue;
}

...
static qboolean FS_CopyFile(const char *fromOSPath, char *toOSPath, char *overwrite, const int owSize) {
	FILE	*f;
	size_t	len, total;
	byte	*buf;
	int		fileCount = 1;
...
void demoAutoSaveLast_f(void) {
	if (trap_Argc() < 2 && FS_CopyFile(va("%s/demos/LastDemo/LastDemo.dm_26", mod), va("%s/demos/%s.dm_26", mod, demoAutoGenerateDefaultFilename()), NULL, 0)) {
		CG_Printf(S_COLOR_GREEN"LastDemo successfully saved\n");
	} else if (FS_CopyFile(va("%s/demos/LastDemo/LastDemo.dm_26", mod), va("%s/demos/%s.dm_26", mod, CG_Argv(1)), NULL, 0)) {
